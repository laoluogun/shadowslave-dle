  ...
  if (savedDate !== null && savedDate !== today) {
   ...
   // Check if more than 1 day has passed (missed a day)
    ...
    if (daysMissed > 1) {
      ...
      const total = JSON.parse(localStorage.getItem('quote-totalGuesses') || '0')
      const guessCount = JSON.parse(localStorage.getItem('quote-guessCount') || '0')
      localStorage.setItem('quote-gamesPlayed', JSON.stringify(played + 1))
      localStorage.setItem('quote-totalGuesses', JSON.stringify(total + guessCount))
      localStorage.setItem('quote-currentStreak', '0')
    }
