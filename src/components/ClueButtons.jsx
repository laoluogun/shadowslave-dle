...
function ClueButtons({totalGuesses, setActiveClue, activeClue, gameOver}) {
    return (
              <div className="flex gap-3">
                {/* Recipient clue button, only enabled after 2 guesses or after the user guesses correctly */}
                <button
                  onClick={() => (gameOver || totalGuesses >= 2) && setActiveClue(activeClue === 'recipient' ? null : 'recipient')}
                  className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-none border transition-colors
                    ${(gameOver || totalGuesses >= 2)
                ...
                >
                  ...
                  <span className="text-xs text-zinc-400">
                    {(gameOver || totalGuesses >= 2) ? '' : `Unlocks in ${2 - totalGuesses} guess`}
                  </span>
                </button>

                 {/* Display the chapter clue button, which becomes available after 3 guesses or after the user guesses correctly */}
                <button
                  onClick={() => (gameOver || totalGuesses >= 3) && setActiveClue(activeClue === 'chapter' ? null : 'chapter')}
                  className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-none border transition-colors
                ...
                >
                  ...
                  <span className="text-xs text-zinc-400">
                    {(gameOver || totalGuesses >= 3) ? '' : `Unlocks in ${3 - totalGuesses} guess${3 - totalGuesses !== 1 ? 'es' : ''}`}
                  </span>
                </button>
              </div>
        )
