    ...
    return (
        <div className="flex items-center justify-center gap-2"> 
            {modes.map(mode => (
                ...
                className={'group relative py-1 px-1 rounded-none transition hover:bg-zinc-600 cursor-pointer hover:scale-105 duration-150'} 
                >
                    <img
                     src={mode.icon}
                     alt={mode.label} 
                     className={`w-8 h-8 brightness-0 object-contain invert transition-opacity ${location.pathname === mode.path ? 'opacity-100' : 'opacity-50'}`}   
                    />
                {/* Tooltip */}
